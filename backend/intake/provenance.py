...
def detect_inconsistencies(
    ...
    if tsh_res and tsh_res.get("is_abnormal"):
        if has_levothyroxine or "thyroxine" in raw_meds.lower() or "thyronorm" in raw_meds.lower() or "eltroxin" in raw_meds.lower():
            flags.append({
    ...
    if chol_res and chol_res.get("is_abnormal"):
        if has_statin or "atorvastatin" in raw_meds.lower() or "statin" in raw_meds.lower() or "atorva" in raw_meds.lower():
            flags.append({
                "category": "RxNorm Medication Monitoring",
    ...
    if creat_res and creat_res.get("is_abnormal") and (has_nsaid or "nsaid" in raw_meds.lower() or "brufen" in raw_meds.lower() or "combiflam" in raw_meds.lower()):
        flags.append({
